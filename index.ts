...
// --- Imports ---
import { type Channel, Client, Events, GatewayIntentBits, Guild } from 'discord.js';
import fs from 'fs';
...
client.once(Events.ClientReady, (readyClient) => {
    ...
    for (const ch of config.submit_channels) {
        ...
        console.log(`Submit: ${ch.id} -> ${channels.get(ch.id)}; Base ID: #${ch.base_id}`);
    }
...
client.on(Events.MessageCreate, async (message) => {
    ...
    const parsed = parseSubmission(message.content, message.author.username, team);
    if ('error' in parsed && parsed.error) {
        console.log(`Invalid submission format in channel ${team}: ${parsed.message}`);
        await message.react('❌').catch(console.error);
        const replyMsg = await message.reply(
            (parsed.userError || 'Your submission format is invalid. Please ensure it follows the correct format.') +
                ...
                embeds: [
                    {
                        ...
                        description: `#<id> <lat>, <lng> [trial] [road] [field]\n\n**Example:**\n #${lastId + 1} 37.7749 -122.4194 road\n #${lastId + 1} 37.7749 -122.4194 field\n #${lastId + 1} 37.7749 -122.4194 trial\n\n-# Supports ID ranges like #100-110`,
    ...
    for (let i = 1; i < submissions.length; ++i) {
        ...
        if (submissions[i]!.id !== submissions[i - 1]!.id + 1) {
            await message.react('❌').catch(console.error);
            const replyMsg = await message.reply(
                `Submission IDs must be consecutive. Found gap between #${submissions[i - 1]!.id} and #${submissions[i]!.id}.`
            );
            setTimeout(() => {
                replyMsg.delete().catch(() => { });
            }, 5 * 60 * 1000);
    ...
    for (const submission of submissions) {
        ...
        if (logChannel && 'send' in logChannel && typeof logChannel.send === 'function') {
            logChannel.send(
                `➕ ${team}: #${submission.id} by ${submission.user} (${submission.lat}, ${submission.lng})${submission.trial ? ' [Trial]' : ''
    ...
    await message.react('✅').catch(console.error);
    console.log(`Submission(s) added to cache for channel ${team}. Total submissions: ${cache.length}`);
});
...
function saveCacheAsCSV() {
    ...
    const rows: string[] = ['team,id,round,lat,lng,user,reviewer,size,road,field,complexity,quality,hindrances,trial,2x'];
    for (const [team, submissions] of submissionCache.entries()) {
        if (submissions.length > 0) hasSubmissions = true;
        for (const sub of submissions) {
            rows.push(
                `${team},${sub.id},${config.current_round},${sub.lat},${sub.lng},${sub.user},,${sub.road || sub.field ? "n" : ""},${sub.road ? "y" : "n"},${sub.field ? "y" : "n"},,,n,${sub.trial ? "y" : "n"},n`
            );
        }
    }
    ...
    const fileName = `submissions-${new Date().toISOString().replaceAll(":", "_")}.csv`;
    const fullDir = path.join(exportsDir, fileName);
    fs.writeFileSync(fullDir, csvRows, 'utf8');
    if (logChannel && 'send' in logChannel && typeof logChannel.send === 'function') {
        logChannel.send(
            { files: [{ attachment: fullDir, name: fileName }], content: `Cache saved to ${fileName}` }
        );
    }
...
function parseSubmission(
    ...
    if (match && typeof match[1] === 'string' && typeof match[3] === 'string') {
        ...
        if (coordsMatch && typeof coordsMatch[1] === 'string' && typeof coordsMatch[3] === 'string') {
            lat = coordsMatch[1];
            lng = coordsMatch[3];
        } else {
            return {
                ...
                userError: "Couldn't parse the coordinates of your submission. Please ensure they are in the format 'latitude, longitude' or 'latitude longitude'."
            };
        }
        const ids: number[] = [];
        ...
        if (match && typeof match[1] === 'string' && typeof match[2] === 'string') {
            ...
            if (match && typeof match[1] === 'string' && typeof match[2] === 'string') {
                ...
                return {
                    ...
                    userError: "Couldn't parse the ID of your submission. Please ensure it starts with a number or # followed by a number.",
                };
            }
        }
